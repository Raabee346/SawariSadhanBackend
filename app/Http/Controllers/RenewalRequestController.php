...
class RenewalRequestController extends Controller
{
    ...
    /**
     * Clean up invalid/ghost FCM tokens
     * Call this method when FCM returns NotRegistered or InvalidRegistration errors
     */
    public function cleanupGhostTokens(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'ghost_tokens' => 'required|array',
            'ghost_tokens.*' => 'required|string',
        ]);
        ...
        $ghostTokens = $request->ghost_tokens;
        $cleanedCount = 0;

        try {
            // Clean ghost tokens from users table
            $usersCleaned = \App\Models\User::whereIn('fcm_token', $ghostTokens)
                ->update(['fcm_token' => null]);
            
            // Clean ghost tokens from vendors table  
            $vendorsCleaned = \App\Models\Vendor::whereIn('fcm_token', $ghostTokens)
                ->update(['fcm_token' => null]);
            
            $cleanedCount = $usersCleaned + $vendorsCleaned;

            Log::info('Ghost FCM tokens cleaned up', [
                'ghost_tokens_count' => count($ghostTokens),
                'users_cleaned' => $usersCleaned,
                'vendors_cleaned' => $vendorsCleaned,
                'total_cleaned' => $cleanedCount,
                'tokens_preview' => array_map(function($token) {
                    return substr($token, 0, 20) . '...';
                }, array_slice($ghostTokens, 0, 5))
            ]);

            return response()->json([
                'success' => true,
                'message' => 'Ghost tokens cleaned successfully',
                'data' => [
                    'ghost_tokens_count' => count($ghostTokens),
                    'total_cleaned' => $cleanedCount,
                ],
            ...
            Log::error('Error cleaning ghost FCM tokens', [
                'error' => $e->getMessage(),
                'ghost_tokens_count' => count($ghostTokens),
                'trace' => $e->getTraceAsString(),
            ]);
            ...
            return response()->json([
                'success' => false,
                'message' => 'Failed to clean ghost tokens',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
