...
class NepaliDate
{
    ...
    private $end_ne = "2095";
    private $end_en = "2038";
    public $month_name = array('बैशाख', 'जेठ', 'असार', 'साउन', 'भदौ', 'असोज', 'कार्तिक', 'मङ्सिर', 'पुस', 'माघ', 'फाल्गुण', 'चैत');
    ...
    public function validate_ne($year, $month, $day)
    {
        if (!array_key_exists($year, $this->nepali_length)) {
            return 'Invalid Year range. BS year must be between ' . $this->start_ne . ' and ' . $this->end_ne . '. Provided: ' . $year;
        }
        if ($month > 12 || $month < 1) {
            return 'Invalid Month range. Month must be between 1 and 12. Provided: ' . $month;
        }
        if ($day > $this->nepali_length[$year][$month - 1] || $day < 1) {
            return 'Invalid Day. Day must be between 1 and ' . $this->nepali_length[$year][$month - 1] . ' for year ' . $year . ', month ' . $month . '. Provided: ' . $day;
        }
        return TRUE;
    }
    ...
    public function validate_en($year, $month, $day)
    {
        if ($year < $this->start_en || $year > $this->end_en) {
            return 'Invalid Year Range. AD year must be between ' . $this->start_en . ' and ' . $this->end_en . '. Provided: ' . $year;
        }
        if ($month < 1 || $month > 12) {
            return 'Invalid Month Range. Month must be between 1 and 12. Provided: ' . $month;
        }
        ...
        if ($day < 1 || $day > $maxDays) {
            return 'Invalid Day Range. Day must be between 1 and ' . $maxDays . ' for year ' . $year . ', month ' . $month . '. Provided: ' . $day;
        }
        return TRUE;
    }
    ...
    public function get_nepali_date($year, $month, $day)
    {
        ...
        if ($validate !== TRUE) {
            // Remove HTML tags from error message and throw exception instead of die()
            $cleanMessage = strip_tags($validate);
            throw new \InvalidArgumentException($cleanMessage);
        }
     ...
     * @param string $bsDate BS date in format YYYY-MM-DD (e.g., "2082-09-19")
     * @return string AD date in format YYYY-MM-DD (e.g., "2026-01-03")
     */
    public function convertBsToAd($bsDate)
    {
        ...
        $adDate = $this->get_eng_date($year, $month, $day);
        
        return sprintf('%04d-%02d-%02d', $adDate['y'], $adDate['m'], $adDate['d']);
    }
    ...
    {
        $months = array('बैशाख', 'जेठ', 'असार', 'साउन', 'भदौ', 'असोज', 'कार्तिक', 'मङ्सिर', 'पुस', 'माघ', 'फाल्गुण', 'चैत');
        return $months;
    }
